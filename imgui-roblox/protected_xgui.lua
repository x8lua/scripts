...
local function ParentGui(gui)
    -- gethui() returns the executor's hidden UI container — safest option
    local huiOk, hui = pcall(function() return gethui() end)
    if huiOk and hui then
        gui.Parent = hui
        return
    end

    -- protectgui hides the gui from game scripts scanning CoreGui
    local protect = (typeof(protectgui) == "function" and protectgui)

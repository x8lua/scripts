...
function setupContainerMethods(container, parentFrame)
    ...
    function container:Script(name, default, callback)
        ...
        local function runUI()
            ...
            local ok, err = pcall(callback, true, renderSurface)
            if not ok then
        ...
        local function toggle()
            ...
            if state then
                ...
                clearUI()
            pcall(callback, false, renderSurface)
            end
        end
...
-- Create several tabs from a declarative table.
-- Each item is {Name = "Tab name", Build = function(tab) ... end}.
function xGui:CreateTabs(definitions)
    local created = {}
    for _, definition in ipairs(definitions or {}) do
        local name = type(definition) == "string" and definition or definition.Name
        local tab = self:CreateTab(tostring(name))
        created[#created + 1] = tab
        if type(definition) == "table" and type(definition.Build) == "function" then
            definition.Build(tab)
        end
    end
    return created
end

-- Hide or show tab headers and views. Pass true to hide all tabs.
function xGui:HideTabs(hidden, except)
    local allow = {}
    if type(except) == "table" then
        for _, name in ipairs(except) do allow[tostring(name)] = true end
    elseif except then
        allow[tostring(except)] = true
    end
    for _, tab in ipairs(self.Tabs) do
        local visible = not hidden or allow[tab.Name] == true
        tab.Button.Visible = visible
        if hidden and not allow[tab.Name] then
...
--[[
    Key-system add-on for xGui.

    Usage:
        local ImGui = loadstring(game:HttpGet("YOUR_RAW_URL"))()
        ImGui.CreateKeySystem({
            Title = "NEBULA",
            Subtitle = "Enter your access key to continue.",
            Keys = { "DEMO-2026" }, -- Replace with ValidateKey for a server-backed check.
            OnSuccess = function(key)
                -- Create or reveal your main UI here.
            end,
        })
]]
function xGui.CreateKeySystem(options)
    options = options or {}

    local title = tostring(options.Title or "KEY SYSTEM")
    local subtitle = tostring(options.Subtitle or "Enter your access key to continue.")
    local keys = options.Keys or {}
    local validateKey = options.ValidateKey
    ...
    heading.Text = title
    heading.Parent = window.MainFrame
    ...
    local function checkKey(candidate, status)
        ...
        local candidate = tostring(candidate or ""):match("^%s*(.-)%s*$")
        if candidate == "" then
            status.Text = "Status: enter a key first"
            status.TextColor3 = Color3.fromRGB(255, 145, 90)
            return
        end

        busy = true
        status.Text = "Status: checking key..."
        status.TextColor3 = Color3.fromRGB(125, 185, 255)

        local valid = false
        local message = nil
        if type(validateKey) == "function" then
            local ok, result, detail = pcall(validateKey, candidate)
            valid = ok and result == true
            message = detail
        else
            for _, allowedKey in ipairs(keys) do
                if candidate == tostring(allowedKey) then
                    valid = true
                    break
                end
            end
    ...
    -- window:CreateTabs({
    --     {Name = "Main", Build = function(mainTab) mainTab:CreateLabel("Unlocked") end},
    -- })

...
function StacyUI.new(options)
    ...
    self.IntroEnabled = options.Intro == true
    ...
    self.Commands.echo = {
        ...
        Callback = function(arguments, _, ui)
            ui:Log(table.concat(arguments, " "), ui.Style.text)
        end,
    }
...
function StacyUI:_registerBuiltIns()
    -- Replica intentionally has no built-ins. echo is registered during construction.
end
...
function StacyUI:_buildMobileOpenButton()
    ...
    -- A normal tap focuses. Hold for one second to enter drag mode.
    local holdToken = 0
    ...
    local function setDragStyle(active)
        dragging = active
        button.BackgroundColor3 = active and Color3.fromRGB(38, 40, 39) or Color3.fromRGB(55, 58, 57)
        button.BackgroundTransparency = active and 0.25 or 0.5
        button.TextColor3 = active and Color3.fromRGB(150, 255, 170) or Color3.fromRGB(235, 240, 236)
        button.Text = active and "MOVE" or "Stacy"
    end
    ...
    self:_connect(button.InputBegan, function(input)
        ...
        task.delay(1, function()
            if token == holdToken and dragInput == input and button.Parent then
                setDragStyle(true)
            end
        end)
    end)
    self:_connect(button.InputChanged, function(input)
...
function StacyUI:_updateMobileLayout()
...
function StacyUI:_onFocusLost(enterPressed)
    if enterPressed then
        ...
        if shouldRefocus and not self.Destroyed and self.Open then
            task.defer(self.Prompt.CaptureFocus, self.Prompt)
        end
...
getgenv().StacyCMDReplica = StacyUI
getgenv().StacyCMDReplicaConsole = console
return StacyUI
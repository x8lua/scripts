...
`reset` sets the local character humanoid health to zero. `lagdetection` toggles server movement monitoring and uses PurgatoryNotify to show `LAG DETECTED` when at least half of available player roots have velocity at or below `1`; the notice clears below that threshold.
